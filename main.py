...
async def show_info(ctx: discord.ApplicationContext, member: discord.Member = None):
    ...
    embed = discord.Embed(
        ...
        description=f'總發言次數: {user_data["msg_sent"]}\n'
                    f'總進入語音頻道時間: {"{:02d}:{:02d}:{:02d}".format(int(user_data["voice_joined_time"]) // 3600, int(user_data["voice_joined_time"]) % 3600 // 60, int(user_data["voice_joined_time"]) % 60)}\n'
                    f'總語音收聽時間: {"{:02d}:{:02d}:{:02d}".format(int(user_data["voice_listen_time"]) // 3600, int(user_data["voice_listen_time"]) % 3600 // 60, int(user_data["voice_listen_time"]) % 60)}\n'
                    f'總語音說話時間: {"{:02d}:{:02d}:{:02d}".format(int(user_data["voice_speak_time"]) // 3600, int(user_data["voice_speak_time"]) % 3600 // 60, int(user_data["voice_speak_time"]) % 60)}\n'
